...
function parseHTMLColor(color) {
  ...
  if (color.startsWith('#') && color.length === 7) {
    result = parseSixDigitHexa(color);
  } else if (color.startsWith('#') && color.length === 4) {
    result = parseThreeDigitHexa(color);
  } else {
    result = parseSixDigitHexa(PRESET_COLORS[color.toLowerCase()]);
  }
  return {
    r: result[0],
    g: result[1],
...
function parseSixDigitHexa(colorCode) {
  return colorCode
    .substring(1, colorCode.length)
    .match(/.{1,2}/g)
    .map((value) => parseInt(value, 16));
}

function parseThreeDigitHexa(colorCode) {
  return colorCode
    .substring(1, colorCode.length)
    .split('')
