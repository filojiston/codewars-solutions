def travel(r, zipcode):
    ...
    parsed = r.split(',')
    matches = list(filter(bool, [address if address.endswith(
        zipcode) else None for address in parsed]))

    if len(matches) == 0:
        return f'{zipcode}:/'

    house_numbers = [match.split(' ')[0] for match in matches]
    matches = [' '.join(match.split(' ')[1:-2]) for match in matches]

    return f'{zipcode}:{",".join(matches)}/{",".join(house_numbers)}'
...
print(travel(ad, "EX 34345") == "EX 34345:Pussy Cat Rd. Chicago/100")
print(travel(ad, "AA 45521") ==
      "AA 45521:Paris bd. Abbeville,Paris St. Abbeville/674,67")
print(travel(ad, "AE 56215") == "AE 56215:Main Al. Bern/320")
